...
module Recommenders
  module Cooccurrence
    #TODO, experiment with different aggregation strategies e.g. sum/max/mean avg

    MODE_PARAMS = {nn: 5, similarity: 50 }

    def self.recommend_to_user(user_id, mode=:nn, param=nil)
      param = MODE_PARAMS[mode] unless param
      user = User.new user_id
      self.send("#{mode.to_s}_recommendations", user.visits ,param)
    end
    ...
    def self.nn_recommendations(visits, top_nn=5)
      ...
      visits.each do |sme_id|
        sme_ids_and_sims = $redis.zrange "sme_similarity:#{sme_id}" ,top_nn,-1, :with_scores => true
    ...
    def self.similarity_recommendations(visits, threshold=50)
      similarities = []
      visits.each do |sme_id|
        sme_ids_and_sims = $redis.zrangebyscore "sme_similarity:#{sme_id}" ,threshold.to_s ,"+inf", :with_scores => true
        similarities << Hash[sme_ids_and_sims]
      end
